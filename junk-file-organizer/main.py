...
current_directory = r"/Users/mintukrishnan/Downloads/Deepthi"
...
def organize_by_date(path):
    ...
    for i in f:
        ...
        d1 = datetime.date(int(timestamp[:4]), int(timestamp[5:7]),
                           int(timestamp[8:]))
        d2 = datetime.date(int(cur_date[:4]), int(cur_date[5:7]),
                           int(cur_date[8:]))
        d3 = str(d2 - d1)
        d4 = d3.split(",")[0]
        if d4[-4:] == "days":
